...
- [~] **DS-W1 FloatingWidgetService** (v105، 2026-06-20 — build-green، needs-device): `service/FloatingWidgetService` — foreground serviceِ specialUse + overlayِ `TYPE_APPLICATION_OVERLAY` که `FloatingOrb` را روی همه‌ی اپ‌ها هاست می‌کند؛ `ComposeView` با `OverlayLifecycleOwner` (Lifecycle/ViewModelStore/SavedState off-Activity)؛ drag→جابه‌جاییِ پنجره؛ `FloatingWidget` controller (canDraw/permissionIntent/start/stop)؛ مجوزهای SYSTEM_ALERT_WINDOW + FOREGROUND_SERVICE_SPECIAL_USE + manifest service. `compileDebugKotlin` سبز. [funnelِ مجوز در Settings + رفتارِ runtime = on-device.]
...
- [~] **DS-W6 Battery/low-mem** (v103، 2026-06-19 — سیاستِ خالص): `core/power/PowerPolicy` + `PowerState`/`PowerMode`(FULL/CONSERVE/MINIMAL) — `mode` از باتری/شارژ/حافظه (شارژ=FULL مگر memory-pressure→MINIMAL؛ ≤۱۵٪ بی‌شارژ=MINIMAL؛ ≤۳۰٪=CONSERVE)؛ `shouldCollapseToDot` (idle>۸s یا غیرFULL)؛ `wakeWordEnabled` (جز MINIMAL)؛ `degradeAnimations`. ۷ تست. [خواندنِ BatteryManager/ActivityManager + اعمال = on-device (DS-BG4).]
