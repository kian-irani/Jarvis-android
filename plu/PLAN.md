...
- [~] **MX1–MX4 Mesh** (PRD Part 14): تبادلِ مدل + session handoff + universal clipboard + cross-device search. **هسته‌های خالصِ ساخته‌شده:** MX1 `MeshModelRouter` (v99)، MX4 `MeshRoutingPolicy` (v100)، universal clipboard `ClipboardSync` (v101، در DS-C3)، **cross-device search `core/search/FederatedSearch` (v101)** — mergeِ نتایجِ rankedِ چند نود به یک لیستِ یکپارچه (dedup by id با بیشترین score، re-rank، limit)، ۵ تست. [اعزامِ شبکه‌ایِ هر کدام = on-device/mesh.]
