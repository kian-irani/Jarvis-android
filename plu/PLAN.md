...
- [x] **LR6 Configurable Dock (Hotseat)** ✅ (v109، 2026-06-20): dockِ شیشه‌ای (`WorkspaceDock` در `WorkspaceScreen`) پایینِ همه‌ی صفحات رندر شد — اپ‌های `Container.HOTSEAT` تا `dockCount` با دکمه‌ی مرکزیِ Vision همیشه؛ تپ=launch. کنترلِ `DockCountRow` (۴/۵/۶) در بخشِ LAUNCHERِ Settings → `setDockCount` (persist روی `LauncherLayout.dockCount`). compile سبز.
- [x] **LR12 Vision Assistant dock button** ✅ (v109، 2026-06-20): دکمه‌ی مرکزیِ dock با `VisionColors.PlasmaSweep` + آیکنِ `Spark` → `animateScrollToPage(0)` + `onAssistant`(→HOME) = بازکردنِ دستیار (orb + command bar). compile سبز.
- [x] **FV4 Wake word** ✅ (v109، 2026-06-20): `service/WakeWordService` (foreground `microphone` + `@AndroidEntryPoint`) — حلقه‌ی پیوسته‌ی `SpeechRecognizer`ِ on-device که partialها را برای «hey vision»/«ویژن»/… اسکن می‌کند و روی hit `VisionEvent.WakeWord` به `VisionEventBus` می‌زند، سپس re-arm (رفعِ باگِ v20 «میکروفون مدام خاموش/روشن»). بدونِ وابستگیِ Porcupine. `WakeWord.start/stop` + permission FOREGROUND_SERVICE_MICROPHONE + manifest. compile سبز.
- [x] **FV6 Voice provider abstraction** ✅ (v109، 2026-06-20): `voice/TTSEngine` interface + `VoiceProvider`(ANDROID/EDGE_NEURAL/PIPER/ELEVENLABS/OPENAI با پرچمِ onDevice) + `VoiceProviderSelector.select` (تعمیمِ `VoiceRouting`: offline→on-device، نوروال‌خواه→non-on-device، fallbackِ never-silent). compile سبز.
- [x] **NEO7 UI quality pass** ✅ (v109، 2026-06-20): پاسِ کیفیتِ مرتبط با dock — گریدِ `WorkspacePage` حالا `bottom=96.dp` inset دارد تا ردیفِ آخرِ آیکن‌ها زیرِ dockِ جدید پنهان نشود؛ گاترهای side تمیز. compile سبز.
...
### 🟡 MEDIUM — این هفته
- [x] **ORB State Machine** (PRD Part 7) ✅ (v91 منطق + **v94 اتصالِ UI**، 2026-06-19): `core/orb/OrbState` — `enum OrbState`(IDLE/LISTENING/THINKING/SPEAKING/EXECUTING/NOTIFICATION/ERROR/SLEEPING) + `OrbSignals` + **`OrbStateMachine.resolve` خالص** با اولویت: turnِ زنده می‌برد (EXECUTING▸LISTENING▸SPEAKING▸THINKING) ▸ ERROR (فقط idle) ▸ NOTIFICATION ▸ SLEEPING (≥۵دقیقه) ▸ IDLE؛ + `label(state)` (معنا فقط رنگ نیست). ۹ تستِ `OrbStateMachineTest`. **اتصالِ UI (v94):** امضای `VisionOrb(listening:Boolean)` → `VisionOrb(state:OrbState)`؛ هر state حلقه را تنگ/گشاد، haloِ glow را مدوله، و captionِ متمایز با رنگِ خودش نشان می‌دهد (LISTENING=NeonGreen · THINKING/EXECUTING=Cyan · SPEAKING=Violet · NOTIFICATION=Amber · ERROR=DangerRed · IDLE/SLEEPING=TextSecondary)، همه از `VisionColors`ِ state-backed و پشتِ `ThemeStore.animations` (reduced-motion امن). `HudUiState.isThinking` افزوده و در `sendChat` با try/finally دورِ turnِ شبکه ست شد؛ `HomeScreen` با `OrbStateMachine.resolve(OrbSignals(listening,speaking,thinking))` گوی را تغذیه می‌کند. ۳ سیگنالِ زنده وصل شد؛ NOTIFICATION/ERROR/EXECUTING/SLEEPING در ماشین آماده‌اند ولی منبعشان (شمارشِ نوتیف/idle-timer/flagِ tool) needs-device. build+test سبز **۵۲۰ تست**. [ظاهرِ per-state روی دستگاه نیازِ تأییدِ بصری.]
...
- [x] **CTX ContextEngine** (PRD Part 8.2) ✅ (v110، 2026-06-20): `data/context/DeviceContextProvider` (Android: battery/charging/network/locale/time-of-day، بدونِ مجوزِ جدید) snapshotِ `DeviceContext` می‌سازد و `CloudChatRouter.chatWith` آن را با `ContextEngine.buildContextBlock` به system prompt تزریق می‌کند (guarded، "" روی خطا → چت رگرس نمی‌کند). foregroundApp برای Accessibilityِ opt-in خالی می‌ماند. compile سبز.
- [x] **PAO Floating overlay** (PRD Part 8.1) ✅ (v110، 2026-06-20): = `FloatingWidgetService` (DS-W1، موجود) — گویِ شناورِ `TYPE_APPLICATION_OVERLAY` روی همه‌ی اپ‌ها؛ حالا gestureها به‌سبکِ PAO تنظیم شد: **tap→پنلِ سریع، double-tap→voice، long-press→voice**، drag→جابه‌جایی. compile سبز.
- [x] **CF5 Automation/Scheduler** (PRD Part 10) ✅ (v110، 2026-06-20): `data/automation/AutomationScheduler` هسته‌ی `ScheduleEvaluator` را با **`AlarmManager`** (بدونِ dep جدید، `setExactAndAllowWhileIdle` Doze-safe + fallbackِ inexact) به سیستم وصل می‌کند — `schedule` نزدیک‌ترین fire را arm می‌کند، `AutomationReceiver` (Hilt) آتش می‌گیرد، dueها را می‌گیرد و re-arm می‌کند. `clock()` (now+dayStart). receiver در manifest. compile سبز. [اتصالِ task-store ماندگار + اجرای planِ AgentEngine = follow-up.]
- [~] **SRCH AnySearch** (PRD Part 9) [= DS-L4] (v89، 2026-06-19 — هسته‌ی رتبه‌بندیِ خالص): `core/search/SearchRanker` ساخته شد — هسته‌ی خالص و دترمینیستیکِ جستجوی یکپارچه که candidateهای منابعِ ناهمگن را در **یک** لیستِ رتبه‌بندی‌شده با **یک تابعِ scoring** ادغام می‌کند (پس نتایجِ APPS/CONTACTS/MEMORY/WEB/… قابلِ‌مقایسه‌اند). `SearchSource`(۸ منبع با وزن: ACTIONS>APPS>CONTACTS>MEMORY>SETTINGS>MESSAGES>FILES>WEB) · `SearchCandidate`(id/title/source/subtitle/`relevanceBoost` که منبع می‌تواند cosine/usage تزریق کند) · `SearchResult`(score). `textScore` خالص ۰..۱ (exact=1.0 · prefix=0.85 · همه‌ی tokenها=0.7 · overlapِ جزئی scale · subtitle-only=0.3 · case-insensitive) و `rank` = score = textScore×وزنِ منبع + boost، فیلترِ بی‌تطبیق برای queryِ ناخالی، queryِ خالی=browse با وزن+boost، مرتب‌سازیِ **پایدار**، limit. **بدونِ نیازِ embedding** (منبع می‌تواند cosine را به boost بدهد). **۹ تستِ جدید** (`SearchRankerTest`: گرادیانِ exact/prefix/token · overlapِ جزئی و صفر · subtitle-only · فیلترِ بی‌تطبیق · شکستِ tie با وزنِ منبع · لیفتِ boost · browseِ queryِ خالی · limit · پایداری) · build+test سبز **۴۸۸ تست**. **اولین آداپترِ زنده (v95):** جستجوی **App Drawer** از فیلترِ substringِ تخت به `SearchRanker` (منبعِ APPS) ارتقا یافت — هر اپ به `SearchCandidate(id=pkg, title=label, relevanceBoost = usageScore نرمال‌شده×۰.۳)` نگاشت، rank می‌شود و به‌ترتیبِ ranked بازچینش (exact▸prefix▸token + tie-breakِ usage به‌سمتِ اپ‌هایی که واقعاً باز می‌کند). فیکسِ Float/Double در boost. drawer-isolated (بدونِ لمسِ chat/voice). [آداپترهای CONTACTS/MEMORY/SETTINGS/WEB + UIِ یکپارچه = on-device follow-up.]
...
- [x] **LR8 Widget Host** ✅ (v110، 2026-06-20): `data/launcher/LauncherWidgetHost` — می‌پیچد `AppWidgetHost`/`AppWidgetManager`: `startListening`/`stopListening`، `availableProviders`، `allocateWidgetId`، `bindWidget`+`bindPermissionIntent`، `configureIntent`، `createView` (AppWidgetHostView)، `deleteWidget`. compile سبز. [قراردادنِ view روی گرید + persist روی `LauncherItem.widgetId` = follow-upِ UI.] · **LR9 Gestures** ✅ (v110): `GestureMap` به `WorkspaceHomePager` وصل شد — `detectVerticalDragGestures` با thresholdِ ۶۴dp: **swipe-up→App Drawer**، **swipe-down→notifications panel** (reflection روی statusbar)؛ paging افقی دست‌نخورده. compile سبز. · **VB9.1 health dots** — باقی‌مانده.

### 🟢 FUTURE
- [x] **MM Multimodal** (PRD Part 11) ✅ (v111، 2026-06-20): `CloudChatRouter.chatWithImage(message, bytes)` — به اولین providerِ vision-capable (OpenAI/Anthropic/Gemini/xAI/OpenRouter؛ Groq skip) با بدنه‌ی multimodalِ ساخته‌شده با `ImageEncoding` (image_url/source/inline_data per-provider) می‌فرستد و پاسخ را parse می‌کند. مسیرِ متنیِ `chat` دست‌نخورده؛ Private mode هم cloud-vision را می‌بندد. compile سبز.
- [~] **MX1–MX4 Mesh** (PRD Part 14): تبادلِ مدل + session handoff + universal clipboard + cross-device search. **هسته‌های خالصِ ساخته‌شده:** MX1 `MeshModelRouter` (v99)، MX4 `MeshRoutingPolicy` (v100)، universal clipboard `ClipboardSync` (v101، در DS-C3)، **cross-device search `core/search/FederatedSearch` (v101)** — mergeِ نتایجِ rankedِ چند نود به یک لیستِ یکپارچه (dedup by id با بیشترین score، re-rank، limit)، ۵ تست؛ **session handoff `core/handoff/SessionHandoff` (v104)** — `SessionSnapshot`(messages/summary/origin) با encode/decodeِ JSONِ lossless + staleness-check برای از سرگیریِ سشن روی دستگاهِ دیگر، ۴ تست. [اعزامِ شبکه‌ایِ هر کدام = on-device/mesh.]
- [x] **LM On-device inference** ✅ (v112، 2026-06-20): `router/local/LocalModelEngine` interface (load/generate/cancel/unload + isLoaded) + `NoOpLocalModelEngine` (graceful — وقتی native-lib نیست fail می‌کند تا router به cloud/mesh substitute کند، هرگز silent نشود = LM6) + `router/backend/LocalInferenceBackend : Backend`(kind=LOCAL) که فایلِ GGUF را از `LocalModelManager` می‌یابد، load و generate می‌کند. آماده‌ی dropکردنِ libِ JNI/llama.cpp بدونِ تغییرِ router. compile سبز. [خودِ native lib = محیطِ NDK.]
...
- [x] **MCP / A2A** ✅ (v112، 2026-06-20): `core/mcp/McpProtocol` — هسته‌ی MCP (JSON-RPC 2.0): `McpRequest`/`McpResponse`/`McpError`/`McpTool` (`@Serializable`) + `McpTransport` (fun-interface seam) + `McpClient` (initialize/listTools/callTool با idِ monotonic). `core/a2a/A2AMessage` — envelopeِ agent-to-agent (REQUEST/RESPONSE/ERROR با correlation-id + from/to + `request`/`respond`/`error` builders). compile سبز. [transportِ stdio/HTTP + bridgeِ McpTool→VisionTool + routeِ A2A روی mesh = runtime half.]
...
- [x] **VCF-M2 Visual perception pipeline** (§8.1) ✅ (v111، 2026-06-20): `data/perception/VisualPerception.seeScreen(prompt)` + `seeImage` — اسکرین‌شات از `VisionAccessibilityService.captureScreenshot` (Accessibility takeScreenshot API، Android 11+، PNG bytes) → `chatWithImage` (MM)؛ degradeِ گویا وقتی سرویس خاموش/pre-R/بدونِ مدلِ vision. `canTakeScreenshot`+`canRetrieveWindowContent` به config افزوده شد. compile سبز. [OCRِ ML-Kit + camera/gallery = follow-up.]
- [x] **VCF-M3 Audio pipeline + STT abstraction** (§8.2 = `FV6`) ✅ (v111، 2026-06-20): `voice/SpeechToText` interface + `SttEngine`(ANDROID/VOSK/WHISPER/CLOUD با onDevice) + `AudioPerception` (acquisition محلی → transcript به agent، `select` محلی‌اول) + `voice/AndroidSpeechToText` (wrapperِ `SpeechRecognizer` با partial-stream، main-thread، offline-prefer). جداسازیِ acquisition/processing. compile سبز.
- [x] **VCF-M4 Wake word** (§8.2 = `FV4`) ✅ (v111، = `WakeWordService`/v109): foreground-serviceِ `SpeechRecognizer` که «hey vision»/«ویژن» را می‌گیرد و `VisionEvent.WakeWord` به `VisionEventBus` (= gateway) می‌زند، re-arm. compile سبز.
...
- [x] **VCF-B1 Contract mirror** ✅ (v112، 2026-06-20 — blocker حل شد): `brain/contract/models.py` (pydantic 2) آینه‌ی DTOهای **flatِ پروتکلِ DS-F2** است که واقعاً روی wire می‌روند (`VisionRequest`/`VisionResponse`/`Intent`/`DeviceContext`/`Attachment`/`MemoryDto`/`ToolSpec`) — نه typeهای sealedِ داخلی (`ContentPart`/`GraphEvent`) که discriminatorِ FQNِ kotlinx مشکل‌ساز بود (**تصمیمِ محافظه‌کارانه: آینه‌ی قراردادِ flatِ شبکه، بدونِ لمسِ سریال‌سازیِ Kotlin/checkpoint**). نام‌فیلد/پیش‌فرض/املای enum دقیقاً مطابقِ kotlinx. **۷ تستِ conformance** `brain/tests/test_contract.py` (round-tripِ کاملِ request با context · defaultهای minimal · واریانت‌های response · memoryDto · toolSpec · deviceContext) → **pytest سبز ۷/۷**. [nodeهای LangGraphِ Python برای tierِ سنگین = follow-up.]
...
- [~] **DS-F1 brain-core extraction**: انتقالِ router+agent+memory به ماژولِ Kotlinِ ساده + facadeِ `VisionBrain` (`handle/state/remember/recall`)، بدونِ تغییرِ رفتار. **پایه‌ی همه.**
  - [x] **facadeِ `VisionBrain`** ✅ (v82، 2026-06-19): `core/brain/VisionBrain` ساخته شد — یک سطحِ ورودیِ واحد روی `VisionGateway` (front-door → agentِ هر channel) و `MemoryEngine`: `handle(request)`/`handle(text, session, channel)` → `Flow<GraphEvent>` · `resume(session, answer)` · `state()` (sessionهای فعال) · `remember(content, type=FACT, importance, metadata)` · `recall(query, topK=5)`. **delegationِ خالص، بدونِ تغییرِ رفتار** — مسیرِ زنده‌ی `HudViewModel` دست‌نخورده؛ surfaceها/SDK در passهای بعدی (DS-C1/DS-F3) روی آن مهاجرت می‌کنند. **۴ تستِ جدید** (`VisionBrainTest` با mockkِ gateway/memory: ساختِ requestِ MAIN/main، عبورِ مستقیمِ request، deferِ state/resume، deferِ remember/recall با پیش‌فرض‌ها). build+test سبز **۴۲۹**. [انتقالِ فیزیکیِ کد به ماژولِ Gradleِ جدا = DS-F4 (monorepo split)؛ این facade آن را آماده می‌کند.]
- [x] **DS-F2 vision-protocol** ✅ (v84، 2026-06-19): پکیجِ `core/protocol/` ساخته شد — قراردادِ wireِ canonical، **خالصِ Kotlin + kotlinx-serialization، بدونِ وابستگیِ Android/runtime** (KMP-ready برای ماژولِ `vision-protocol`ِ DS-F4). DTOها: `VisionRequest` (envelopeِ غنی: text/sessionId/`Channel`/locale/`Intent`?/`DeviceContext`?/`List<Attachment>` + `toGatewayRequest()` که به requestِ مینیمالِ in-processِ gateway تنگ می‌کند)، `VisionResponse` (text/intent/finished/`awaitingConfirmation`ِ HIL/error)، `Intent` (name/confidence/slots + `CHAT` fallback)، `DeviceContext` (foregroundApp/battery/charging/network/locale/timeOfDay/… همه nullable برای CTX/DS-B2/DS-W5)، `Attachment`+`AttachmentKind` (base64 یا uri — KMP-safe، نه ByteArray/Bitmap)، `MemoryDto` (شکلِ wireِ CF4/DS-B3 برای syncِ DS-C3)، و `VisionEventDto` + مَپرهای **lossless** `VisionEvent.toDto()`/`toEvent()` (آینه‌ی flatِ sealedِ VCF-R2 برای عبور از network-plane). enumهای پاکِ موجود (`Channel`/`MemoryType`/`EventKind`) دوباره ساخته نشدند، reuse شدند (در DS-F4 به این پکیج منتقل می‌شوند). **purely additive** — مسیرِ زنده‌ی `HudViewModel` دست‌نخورده؛ DS-C1/DS-C2/DS-F3 روی این types سوار می‌شوند. **۱۰ تستِ جدید** (`VisionProtocolTest`: round-tripِ کاملِ request با context · decodeِ مینیمال با defaults · toGatewayRequest · response (finished/interrupt/error) · intent defaults · memoryDto round-trip/defaults · round-tripِ lossless هر ۵ واریانتِ event دوطرفه · eventDto JSON round-trip · payloadِ گمشده→defaultِ امن) · build+test سبز **۴۴۶ تست**. [انتقالِ فیزیکی به ماژولِ Gradleِ جدا = DS-F4؛ این protocol آن را آماده می‌کند، هم‌سطحِ DS-F1.]
- [~] **DS-F3 vision-sdk** (v85، 2026-06-19 — in-process plane کامل): پکیجِ `core/sdk/` — کلاینتِ **transport-agnostic** که هر surface (widget/launcher/desktop) علیهِ آن کد می‌زند و فقط typeهای protocolِ DS-F2 می‌بیند (هیچ `GraphEvent`ِ runtime نشت نمی‌کند). `interface VisionSdk`: `send(VisionRequest|text)`/`resume(sessionId, answerstring)` → `Flow<VisionResponse>` · `remember`/`recall(): List<MemoryDto>` · `sessions()`. **`InProcessVisionSdk(VisionBrain)` (= DS-C1):** به facadeِ هم‌پروسه bind می‌شود — استریمِ `GraphEvent` را به responseها project می‌کند، رشته‌ی تأییدِ ساده را به `VisionMessage(USER)`ِ موردِنیازِ resume می‌پیچد، و `Recalled`→`MemoryDto` (relevance در `metadata["score"]`) می‌نگارد؛ delegationِ خالص بدونِ تغییرِ رفتارِ Brain. **`ResponseProjector` خالص** = درزِ transport-agnostic: `GraphEvent`(غیرسریالایزبل) → `VisionResponse`(wire): tokenها تجمیعِ cumulative (finished=false) · `Interrupted`→`awaitingConfirmation` (HIL) · `Done`→پاسخِ نهایی (متنِ stream یا آخرین پیامِ assistant) · `Failed`→error؛ eventهای داخلیِ node/tool چیزی emit نمی‌کنند. `fold(list)` دترمینیستیک + `asResponses(Flow)` با همان منطق (stateِ تازه per-collection). **۱۰ تستِ جدید** (`ResponseProjectorTest` ۶: tokenهای cumulative · استخراجِ آخرین assistant در Done بدونِ token · interrupt→awaiting · failure→error+partial · eventهای داخلی بی‌emit · parityِ flow با fold؛ `InProcessVisionSdkTest` ۴: projectِ send · پیچیدنِ resume به USER · map به MemoryDto با score · passthroughِ remember/sessions) · build+test سبز **۴۵۶ تست**. [آداپترِ networkِ کلاینت (روی `/v1/stream`ِ DS-C2/v80) = needs-network follow-up؛ in-process plane (DS-C1) کامل.]
...
- [~] **DS-W1 FloatingWidgetService** (v105، 2026-06-20 — build-green، needs-device): `service/FloatingWidgetService` — foreground serviceِ specialUse + overlayِ `TYPE_APPLICATION_OVERLAY` که `FloatingOrb` را روی همه‌ی اپ‌ها هاست می‌کند؛ `ComposeView` با `OverlayLifecycleOwner` (Lifecycle/ViewModelStore/SavedState off-Activity)؛ drag→جابه‌جاییِ پنجره؛ `FloatingWidget` controller (canDraw/permissionIntent/start/stop)؛ مجوزهای SYSTEM_ALERT_WINDOW + FOREGROUND_SERVICE_SPECIAL_USE + manifest service. `compileDebugKotlin` سبز. **+ funnel + integration (v106):** بخشِ «FLOATING WIDGET» در SettingsHubScreen (Enable/Disable/Overlay-permission via `FloatingWidget`)؛ سرویس حالا OrbState را با `OrbStateMachine.resolve` از سیگنال‌های زنده می‌سازد — نوتیفِ unread (`VisionNotificationService.badges`→`NotificationBadges.total`) → NOTIFICATION، و باتریِ کم (`PowerPolicy.shouldCollapseToDot` روی `ACTION_BATTERY_CHANGED`) → collapse-to-dot. build-green. [رفتارِ runtime = on-device.]
- [~] **DS-W2 Orb state machine UI** [= ORB] (v105، 2026-06-20 — build-green): `ui/widget/FloatingOrb` → `OrbVisual` که با `OrbState` رنگ/glow/اندازه می‌گیرد (idle breathing / listening / thinking / speaking glow / sleeping→collapse-to-dot)، sweepِ چرخان، honorِ reduced-motion (`ThemeStore.animations`)، ۱۸۰–۲۲۰ms. پالتِ azure→violetِ `VisionColors`. [تأییدِ بصری = on-device.]
- [~] **DS-W3 Gestures** (v102، 2026-06-19 — نگاشتِ خالص): `core/gesture/GestureMap` + `Gesture`/`GestureAction` — `DEFAULTS` طبقِ اسپک (tap→quick prompt، long-press→voice، swipe-up→expand، swipe-side→quick actions، double-tap→repeat-last)؛ override per-gesture (NONEِ صریح = غیرفعال)؛ `bind` immutable. ۶ تست. **+ wired در widget (v105):** `FloatingOrb` ژست‌های tap/double-tap/long-press را با `pointerInput`/`detectTapGestures` می‌گیرد و از همین `GestureMap.actionFor` به `GestureAction` نگاشت می‌کند → سرویس expand/launch می‌کند. build-green. [تشخیصِ swipe + اجرای کاملِ اکشن = on-device.]
- [~] **DS-W4 Mini-panels** (v103، 2026-06-19 — هسته‌ی quick-capture): `core/capture/QuickCapture` + `Capture`(Note/Reminder) — متنِ ورودیِ پنلِ capture را مسیریابی می‌کند: عبارتِ زمان‌دار/keyworded → `Reminder` (با schedule از `ScheduledCommandParser`/AGT-SCHED) → scheduler؛ بقیه → `Note` → Memory؛ نرمال‌سازیِ prefixِ «remind me to»/«یادم بنداز». ۶ تست. **+ Automation panel (v104):** `core/automation/MacroEngine` + `Macro`/`MacroStep` — ماکروی نام‌دارِ کاربر (دنباله‌ی اکشن، record/append/validate/plan؛ ماکروی disabled/نامعتبر = no-op، نه اجرای ناقص)، `@Serializable`، ۶ تست. [Smart Search (DS-L4 ✅)، Recent-memory = پنل‌های دیگر؛ persist/route + ضبطِ اکشنِ واقعی = on-device.] **+ panel UI (v105):** `FloatingOrb.MiniPanel` — پنلِ شیشه‌ای (`glassPanel`) با اکشن‌های Ask/Voice/Note/Open/Close (آیکنِ `VisionIcons`، تارگتِ ۶۴dp، بدونِ emoji)، AnimatedVisibility با scale/fade. build-green. [ورودیِ متنِ کامل + route به Memory/Scheduler = on-device.]
- [x] **DS-W5 Context awareness** ✅ (v111، 2026-06-20): `VisionAccessibilityService` حالا از `TYPE_WINDOW_STATE_CHANGED` نامِ بسته‌ی اپِ foreground را در `foregroundPackage` نگه می‌دارد؛ `DeviceContextProvider.snapshot` آن را در `DeviceContext.foregroundApp` می‌گذارد → `ContextEngine` به promptِ چت تزریق می‌کند («the user is in <app>»). مشترک با CTX/DS-B2. compile سبز.
- [~] **DS-W6 Battery/low-mem** (v103، 2026-06-19 — سیاستِ خالص): `core/power/PowerPolicy` + `PowerState`/`PowerMode`(FULL/CONSERVE/MINIMAL) — `mode` از باتری/شارژ/حافظه (شارژ=FULL مگر memory-pressure→MINIMAL؛ ≤۱۵٪ بی‌شارژ=MINIMAL؛ ≤۳۰٪=CONSERVE)؛ `shouldCollapseToDot` (idle>۸s یا غیرFULL)؛ `wakeWordEnabled` (جز MINIMAL)؛ `degradeAnimations`. ۷ تست. **+ wired (v106):** `FloatingWidgetService` یک `BroadcastReceiver`ِ `ACTION_BATTERY_CHANGED` ثبت می‌کند، `PowerState` می‌سازد و `PowerPolicy.shouldCollapseToDot` → orb را به dot جمع می‌کند. build-green. [اعمالِ کاملِ wake-word/animation budget = on-device (DS-BG4).]
...
- [x] **DS-L2 Adaptive home** ✅ (v98، 2026-06-19): `core/launcher/AdaptiveHomePredictor` — پیشنهادِ پویای اپ بر اساسِ الگوی **زمانِ-روز** (هر launch با hourOfDay) + recency-decay؛ اپ‌های هم‌ساعتِ الان boost می‌گیرند ولی اپِ پرکاربرد همیشه در ردیف می‌ماند (هرگز خالی). circular-hour (wrap نیمه‌شب)، exclude برای pinnedها، topN. خالص و JVM-tested (۶ تست). متمایز از `AppUsageRanker`/DS-L5 (که globalِ freq×recency است). [اتصالِ UsageStats + رندرِ ردیف = on-device.]
- [x] **DS-L3 Edit-mode AI optimize** (v93، 2026-06-19 — undo/redo خالص): `data/launcher/LayoutHistory` ساخته شد — استکِ undo/redoِ خالص و immutable روی `LauncherLayout` (که خودش immutable/`@Serializable` است)، پس هر mutationِ edit-mode (drag/folder/«clean home»/«optimize productivity») برگشت‌پذیر است. سمانتیکِ استانداردِ editor: `record(next)` presentِ جدید را push و redo را پاک می‌کند؛ `record`ِ هم‌سانِ present = no-op (بدونِ ورودیِ مرده)؛ `undo`/`redo` کرسر را حرکت و layoutِ اعمال‌شدنی را برمی‌گردانند (null در انتها)؛ `capacity`(۵۰) گذشته را کران‌دار می‌کند. `canUndo`/`canRedo`. **۸ تستِ جدید** (`LayoutHistoryTest`: fresh بدونِ undo/redo · record→undo · redo · پاک‌شدنِ redo با editِ جدید · no-opِ هم‌سان · undo/redoِ چندمرحله‌ای · کران‌داریِ capacity با eviction · ردِ capacity<1) · build+test سبز **۵۲۰ تست**. **اتصالِ زنده (v96):** `LauncherStore` حالا `LayoutHistory` را در choke-pointِ واحدِ `update()` ثبت می‌کند؛ `undo()`/`redo()`/`canUndo`/`canRedo` افزوده شد و از طریقِ مسیرِ `apply()` (بدونِ re-record) layout را persist/emit می‌کنند. شیتِ **Edit Home** دو دکمه‌ی شیشه‌ای **Undo/Redo** گرفت (≥۴۸dp، dim+غیرفعال وقتی چیزی برای step نیست، باز می‌ماند تا چند step سریع باشد، آیکنِ auto-mirrored). drawer/launcher-isolated. **اکشنِ Optimize (v97):** `LauncherOps.autoArrange` (خالصِ تست‌شده) اپ‌های top-levelِ workspace را بر اساسِ category به فولدر گروه می‌کند (هر category با ≥۲ اپ یک فولدر؛ اپ‌های uncategorized دست‌نخورده می‌مانند — محافظه‌کارانه)، فولدرها در اولین سلولِ آزاد جا می‌گیرند (با رشدِ صفحه در صورتِ پر بودن)، فولدرها/ویجت‌ها/فرزندانِ موجود دست‌نخورده؛ `LauncherStore.autoArrange(categoryOf)` فقط idِ فولدر می‌زند و persist می‌کند (undoable مثلِ هر edit)؛ `LauncherViewModel.autoArrange` categoryِ هر اپ را از `PackageManager` به نامِ فولدرِ `AppGroupCategory` نگاشت می‌کند؛ شیتِ Edit Home دکمه‌ی **«Auto-organize apps»** گرفت. **۶ تستِ جدید** (`LauncherAutoArrangeTest`: گروه‌بندی به فولدرِ category · رهاکردنِ category تک‌اپی · جاگیریِ فولدر در سلولِ آزادشده + فرزندِ folder-local · بدونِ qualifying→بدونِ تغییر · نادیده‌گرفتنِ uncategorized · دست‌نخوردنِ فولدر/فرزندِ موجود) · build+test سبز **۵۲۶ تست**. **DS-L3 کامل.** [ظاهر/رفتار روی دستگاه نیازِ تأییدِ بصری.]
...
- [x] **DS-B3 Memory deepen** [CF4 ✅ پایه] ✅ (v98، 2026-06-19): `core/memory/SessionMemory` — نیمه‌ی short-termِ گمشده. پنجره‌ی محدودِ نوبت‌ها (`VisionMessage`)؛ با عبور از `maxTurns`، نوبت‌های قدیمی‌تر از `keepRecent` به `compactable()` می‌روند تا `ConversationSummarizer`ِ موجود (VCF-MEM1، مدل‌محور) فشرده‌شان کند، سپس `applySummary` آن‌ها را drop و خلاصه را در `window()` به‌صورتِ SYSTEM-message تزریق می‌کند → `MemoryEngine` آن را SEMANTIC ذخیره. graceful: خلاصه‌ی خالی = no-op (نوبت‌ها حفظ می‌شوند). خالص و JVM-tested (۷ تست). [حلقه‌ی short-term↔long-term حالا کامل: window + summarizer + engine.]
- [~] **DS-B4 Task engine** (v90، 2026-06-19 — هسته‌ی resilience): `core/task/TaskResilience` ساخته شد — ستونِ عامِ مقاوم‌سازیِ workflowِ زنجیره‌ای که agent/`ToolCaller` رویش سوار می‌شوند (عمداً نه از HTTP/tool/model خبر دارد). **`RetryPolicy` خالص:** `maxAttempts`/`baseDelayMillis`/`multiplier`/`maxDelayMillis`/`retryOn` + `shouldRetry(attempt, error)` (تا exhaust یا خطای nonه‌retryable) + `backoffMillis(attempt)` (نماییِ کلمپ‌شده). **executorهای نازکِ suspend:** `withRetry(policy){block}` (خطای transient را با backoff تکرار، اولین موفقیت را برمی‌گرداند، در exhaust آخرین خطا را rethrow) و `withFallback(listOf({cloud},{local}))` (degradationِ graceful: اولین alternativeِ موفق؛ همه‌شکست→آخرین خطا با suppressedِ قبلی‌ها؛ لیستِ خالی=خطای برنامه‌نویس). policy خالصِ unit-testable است و executorها با `delay` → با ساعتِ مجازیِ `runTest` تست می‌شوند (بدونِ انتظارِ واقعی/دستگاه). **۹ تستِ جدید** (`TaskResilienceTest`: رشدِ نمایی+cap · stopِ shouldRetry در maxAttempts + احترام به retryOn · ردِ maxAttempts<1 · موفقیت بعد از transient · rethrow بعد از exhaust · بدونِ retryِ خطای nonه‌retryable · اولین موفقِ fallback (c امتحان‌نشده) · rethrowِ آخرین با suppressed · ردِ fallbackِ خالی) · build+test سبز **۴۹۷ تست**. [اتصالِ `withRetry`/`withFallback` به stepهای واقعیِ `ToolNode`/`AgentEngine` (cloud→local، tool→degraded) = follow-up.]
...
- [~] **DS-X1 Plugin SDK** (v98، 2026-06-19 — قرارداد + گیت کامل): `core/plugin/PluginRegistry` — لایه‌ی پلاگین روی `ToolRegistry`/`VisionTool`. `PluginManifest` (id/name/version + `Capability`های درخواستی) + `VisionPlugin.tools(): List<PluginTool>` (هر ابزار با capهای موردنیازش). `install(plugin, granted)` **fail-closed** اگر manifest چیزی خارج از grant بخواهد؛ ابزارهای over-reach drop می‌شوند (نه expose)؛ بازماندگان به `toolRegistry()` می‌پیوندند. upgrade-in-place (keyed by id) + uninstall. sandboxِ capability-based (NETWORK/MESSAGING/CONTACTS/…). خالص، ۶ تست. [بارگذاریِ کدِ پلاگین از APK/DEX + UIِ مجوز = on-device؛ هم‌راستا با MCP/DS-X.]
- [~] **DS-X2 Deployment** (v101، 2026-06-19 — feature flags کامل): `core/flags/FeatureFlags` + `FeatureFlag`(default/channelOverrides/rolloutPercent) + `ReleaseChannel`(DEV/BETA/PROD) — گیتِ خالصِ fail-closed (flagِ ناشناس = off)؛ override per-channel؛ rolloutِ درصدی **دترمینیستیک** با bucketِ FNV-1aِ پایدار از key+installId (همان install همیشه همان جواب). ۸ تست (شاملِ توزیعِ ~۳۰٪ روی ۲۰۰۰ install). [CI/CD per-module + packagingِ desktop (MSI/exe) + deployِ brain-server = کارِ زیرساخت/on-host؛ این هسته‌ی flag را برای کانال‌ها فراهم می‌کند.]
...
- [~] **CF5 Scheduler / Automation** (`core/automation/`) (v99، 2026-06-19 — هسته‌ی خالص کامل): `ScheduleEvaluator` + `Schedule`(Once/Interval/DailyAt) + `ScheduledTask` — تصمیمِ خالصِ «کدام تسک الان due است» + `nextFire`. بدونِ clock-dep (یک `Clock`-snapshot تزریقی شاملِ nowMillis + dayStart برای حلِ DailyAtِ timezone-aware)؛ Interval اولین fire را یک period بعدِ anchor می‌زند و missed-slotها را skip می‌کند (بدونِ catch-up storm)؛ Once یک‌بار؛ DailyAt روزی یک‌بار. ۸ تست (یک باگِ off-by-period حین TDD گرفته و رفع شد). **+ rule engine (v104):** `core/automation/AutomationEngine` + `AutomationRule`/`AutomationTrigger`(OnEvent/OnSchedule) — لایه‌ی «when X do Y» که triggerهای رویدادیِ VCF-R2 و زمانیِ CF5 را یکی می‌کند: `firedByEvent` (kind+condition)، `firedBySchedule` (due via ScheduleEvaluator)، `actions` (flatten). ۶ تست. [اتصال به WorkManager/AlarmManager + اجرای planِ AgentEngine = on-device (DS-BG).]
...
- [~] **PAW — Proactive Writing Assist** (v103، 2026-06-19 — گیتِ خالص): `core/writing/WritingAssist` + `FieldContext`/`AssistDecision` — `evaluate` تصمیم می‌گیرد آیا اصلاً پیشنهادِ بازنویسی بدهد (فقط فیلدِ editable + غیرحساس [نه password/OTP/card] + ≥minWords محتوا) و `improvePrompt` را می‌سازد — قبل از مزاحمتِ مدل/کاربر. ۶ تست. [خواندنِ فیلدِ فعال via Accessibility + بازنویسیِ مدل + نوشتنِ ACTION_SET_TEXT = on-device.]
- [~] **PAB — Browser/YouTube automation** (v103، 2026-06-19 — سازنده‌ی URI خالص): `core/web/WebIntentBuilder` + `IntentSpec` — ساختِ خالصِ URIها: `youtubeSearch` (results?search_query)، `youtubeApp` (vnd.youtube://)، `webSearch` (google)، `open` (تشخیصِ URL/domain/free-text)؛ percent-encodingِ RFC-3986 (فاصله→%20). ۷ تست. [پیچیدنِ هر `IntentSpec` در `Intent` واقعی + پخشِ نتیجه = on-device `WebTool`/`MediaTool` (اجرای واقعی، نه ادعای دروغ).]
...
- [~] **PAS — Suggestion Engine** (v99، 2026-06-19 — هسته‌ی خالص): `core/proactive/SuggestionEngine` — از candidateهای پیشنهاد (relevance + kind + زمان) آن‌هایی را که الان باید نشان داده شوند انتخاب می‌کند، **بدونِ مزاحمت**: گیتِ `minRelevance` + cooldownِ per-kind + سقفِ نرخِ پنجره‌ای (`maxPerWindow`/`windowMillis`) + حداکثر یکی از هر kind در هر tick؛ rank بر اساسِ relevance، tie با id. `SuggestionPolicy` قابل‌تنظیم (حساسیت). ۷ تست. [جمع‌آوریِ سیگنال از MON/Timeline + رندر در overlay = on-device.]
...
- [x] **LM5 Privacy Mode** ✅ (v102، 2026-06-19): `core/privacy/PrivacyPolicy` — local-only هر task را PRIVATE می‌کند، `allowsCloud=false`، و `sanitize` هر `RouteDecision.Cloud` را به `Unavailable` تنزل می‌دهد (belt-and-suspenders روی MX4)؛ `label` برای Trust gate. خالص، به `core/mesh`(Sensitivity/RouteDecision) وصل. ۶ تست. **+ UI toggle (v106):** «Private mode» در بخشِ PRIVACY MONITORِ Settings + `VisionSettings.privacyLocalOnly` (persist). **+ enforcement end-to-end (v107):** `CloudChatRouter.chat` در ابتدا اگر `settings.privacyLocalOnly` روشن باشد، **هرگز به cloud نمی‌رود** و پیامِ گویا برمی‌گرداند (gateِ guardشده، default خاموش → مسیرِ چتِ موجود دست‌نخورده). Private mode حالا کامل کار می‌کند: toggle → setting → router-gate. build-green.
