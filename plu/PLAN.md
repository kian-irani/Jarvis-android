...
- [ ] **LR2 Workspace grid + pages**: گریدِ واقعیِ هوم که از مدل رندر می‌شود (اپ‌های pin‌شده روی سلول‌ها)، `HorizontalPager` برای صفحات + page indicators + swipe چپ/راست. (جایگزینِ Column اسکرولی.)
...
- [ ] **🚨 FIX-VOICE/LANG (باگ v20، تست روی دستگاه واقعی لازم)**:
  ...
  - [ ] **FV2 voice picker واقعی**: انتخاب صدا/جنسیت از `TextToSpeech.voices` (نه فقط rate/pitch) + دکمه‌ی TEST VOICE + persist. (آیتمِ بازِ Phase 7.)
  - [~] **FV3 درک فارسی + code-switch** (v28، 2026-06-15 auto-rp): بخشِ نوشتنِ Finglish رفع شد — قاعده‌ی سخت‌گیرانه‌ی `SCRIPT_RULE` در پرامپتِ سیستمی (`CloudChatRouter`): «هر زبان با خطِ بومیِ خودش؛ فارسی با حروفِ فارسی، هرگز ترنسلیتریشنِ لاتین/Finglish». [باقی‌مانده: STT چندلوکیلِ خودکار + ترنسکرایبِ کلماتِ انگلیسیِ داخلِ فارسی.]
