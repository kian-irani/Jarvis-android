...
> ریپو: `kian-irani/Jarvis-android` · **آخرین نسخه: v96** (2026-06-19).
...
- [~] **DS-L3 Edit-mode AI optimize** (v93، 2026-06-19 — undo/redo خالص): `data/launcher/LayoutHistory` ساخته شد — استکِ undo/redoِ خالص و immutable روی `LauncherLayout` (که خودش immutable/`@Serializable` است)، پس هر mutationِ edit-mode (drag/folder/«clean home»/«optimize productivity») برگشت‌پذیر است. سمانتیکِ استانداردِ editor: `record(next)` presentِ جدید را push و redo را پاک می‌کند؛ `record`ِ هم‌سانِ present = no-op (بدونِ ورودیِ مرده)؛ `undo`/`redo` کرسر را حرکت و layoutِ اعمال‌شدنی را برمی‌گردانند (null در انتها)؛ `capacity`(۵۰) گذشته را کران‌دار می‌کند. `canUndo`/`canRedo`. **۸ تستِ جدید** (`LayoutHistoryTest`: fresh بدونِ undo/redo · record→undo · redo · پاک‌شدنِ redo با editِ جدید · no-opِ هم‌سان · undo/redoِ چندمرحله‌ای · کران‌داریِ capacity با eviction · ردِ capacity<1) · build+test سبز **۵۲۰ تست**. **اتصالِ زنده (v96):** `LauncherStore` حالا `LayoutHistory` را در choke-pointِ واحدِ `update()` ثبت می‌کند؛ `undo()`/`redo()`/`canUndo`/`canRedo` افزوده شد و از طریقِ مسیرِ `apply()` (بدونِ re-record) layout را persist/emit می‌کنند. شیتِ **Edit Home** دو دکمه‌ی شیشه‌ای **Undo/Redo** گرفت (≥۴۸dp، dim+غیرفعال وقتی چیزی برای step نیست، باز می‌ماند تا چند step سریع باشد، آیکنِ auto-mirrored). drawer/launcher-isolated. [اکشن‌های «clean home»/«optimize productivity» (`AppGrouper`+`LauncherOps`) = follow-up؛ undo/redo حالا کامل و قابلِ‌تست روی دستگاه.]
