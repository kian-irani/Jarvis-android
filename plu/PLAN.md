...
> ریپو: `kian-irani/Jarvis-android` · **آخرین نسخه: v93** (2026-06-19).
...
### 🟡 MEDIUM — این هفته
- [~] **ORB State Machine** (PRD Part 7) (v91، 2026-06-19 — منطقِ خالص): `core/orb/OrbState` ساخته شد — `enum OrbState`(IDLE/LISTENING/THINKING/SPEAKING/EXECUTING/NOTIFICATION/ERROR/SLEEPING) + `OrbSignals`(listening/speaking/thinking/executing/hasNotification/error/idleMillis) + **`OrbStateMachine.resolve` خالص** که سیگنال‌های runtime را به state نگاشت می‌کند با اولویت: turnِ زنده همیشه می‌برد (EXECUTING▸LISTENING▸SPEAKING▸THINKING) ▸ سپس ERROR (فقط وقتی idle، تا turnِ زنده را پنهان نکند) ▸ NOTIFICATION ▸ SLEEPING (idle طولانی ≥۵دقیقه) ▸ IDLE؛ به‌علاوه `label(state)` (معنا هرگز فقط روی رنگ سوار نیست). هم‌سبکِ `commandBarMode`ِ خالص، framework-free و JVM-testable. **۹ تستِ جدید** (`OrbStateMachineTest`: no-signal→IDLE · executing بر همه · listening>speaking>thinking · thinking در flight · error فقط idle · notification با تسلیم به error · sleepِ idle طولانی با تسلیم به notification · labelِ ناخالی برای هر state) · build+test سبز **۵۰۶ تست**. [اتصالِ `VisionOrb` به `OrbState` (جایگزینِ boolean `listening`) + انیمیشن/رنگِ هر state با احترام به reduced-motion = needs-device UI follow-up؛ ماشینِ حالتِ خالص آماده و تغذیه‌شدنی از `isSpeaking`/`isListening`/decision.]
