    ...
    private suspend fun handleWs(raw:String) {
        try {
            ...
            when(obj["type"]?.jsonPrimitive?.content) {
                "heartbeat"   -> { val nid=obj["node_id"]?.jsonPrimitive?.content?:return; val m=obj["metrics"]?.let{json.decodeFromJsonElement<NodeMetrics>(it)}; _events.emit(BrainEvent.NodeUpdated(ApiNode(nid,nid,metrics=m))) }
                ...
                "connected"   -> obj["nodes"]?.jsonArray?.forEach{n->_events.emit(BrainEvent.NodeUpdated(json.decodeFromJsonElement<ApiNode>(n)))}
            }
