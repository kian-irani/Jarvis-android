...
private val IDLE_TEXTS = listOf(
    ...
    "Node mesh connected — 2 active nodes.",
    "Groq LLaMA-3.3-70B ready. Key rotation active.",
...
class HudViewModel @Inject constructor(
            ...
            brain.events.collect { event ->
                when (event) {
                    is BrainEvent.Connected   -> {
                        ...
                        typeText("Brain server online. All systems nominal.")
                        stopIdle()
                    }
                    is BrainEvent.Disconnected -> {
                        _state.update { it.copy(brainOnline = false) }
                        addLog("Brain disconnected — reconnecting...", "warn")
                        startIdle()
                    }
    ...
    fun sendChat() {
        ...
        viewModelScope.launch {
            typeText("Processing...")
            brain.chat(msg).onSuccess { resp ->
                typeText(resp.response)
                addLog("AI: ${resp.duration_ms}ms via ${resp.node_used}", "ok")
            }.onFailure { e ->
    ...
    fun toggleListening() {
        ...
        if (now) { addLog("Voice activated", "ok"); typeText("Listening...") }
        else     { addLog("Voice deactivated", "info") }
    }
    ...
    private fun addLog(msg: String, level: String) {
        val e = LogEvent(logFmt.format(Date()), msg,
            when(level) { "ok"->"OK"; "warn"->"WARN"; "err"->"ERR"; else->"INFO" }
                .let { EventLevel.valueOf(it) })
