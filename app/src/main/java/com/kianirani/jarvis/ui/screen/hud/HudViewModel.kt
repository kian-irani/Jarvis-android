...
class HudViewModel @Inject constructor(private val brain:BrainRepository) : ViewModel() {
    ...
    private var typeJob:Job?=null; private var idleJob:Job?=null
    ...
    private fun startWaveform() = viewModelScope.launch {
        ...
        while(isActive){
            ...
            val bars=List(40){i->if(listening){val dist=abs(i-20f)/20f;((1f-dist*.5f)*(.3f+Math.random().toFloat()*.7f)).coerceIn(.05f,1f)}else{(0.08f+0.06f*sin(t+i*0.3f)).coerceIn(.02f,.2f)}}
            _state.update{it.copy(waveformAmplitudes=bars)}; t+=0.15f; delay(60)
        }
    }
    ...
    private fun connectBrain() {
        ...
        viewModelScope.launch {
            brain.events.collect{event->when(event){
                is BrainEvent.Connected   -> { _state.update{it.copy(brainOnline=true,groqOnline=true)}; addLog("Brain connected","ok"); typeText("Brain online. All systems nominal."); stopIdle() }
                is BrainEvent.Disconnected -> { _state.update{it.copy(brainOnline=false)}; addLog("Disconnected — reconnecting...","warn"); startIdle() }
    ...
    private fun updateNode(n:com.kianirani.jarvis.data.repository.ApiNode) {
        _state.update{state->
            val m=n.metrics; val ex=state.nodes.firstOrNull{it.id==n.node_id}
            val updated=NodeInfo(n.node_id,n.name,ex?.ip?:"",if(n.name.contains("tr")||n.name.contains("trvps"))"BRAIN" else "NODE",
                n.status=="online",m?.cpu_percent?.toInt()?:ex?.cpu?:0,
            ...
            state.copy(nodes=nodes,nodesOnline=nodes.count{it.online},
                ...
                brainRam=if(isBrain&&m!=null&&m.ram_total_gb>0)m.ram_used_gb/m.ram_total_gb*100 else state.brainRam,
    ...
    fun sendChat() {
        ...
        viewModelScope.launch {
            typeText("Processing...")
            brain.chat(msg).onSuccess{resp->typeText(resp.response);addLog("AI: ${resp.duration_ms}ms","ok")}
                           .onFailure{e->typeText("Error: ${e.message}");addLog("Failed: ${e.message}","err")}
        }
    }

    fun toggleListening() {
        val now=!_state.value.isListening; _state.update{it.copy(isListening=now)}
        if(now){addLog("Voice activated","ok");typeText("Listening...")} else addLog("Voice off","info")
    }

    private fun typeText(text:String) {
        typeJob?.cancel(); typeJob=viewModelScope.launch {
            _state.update{it.copy(jarvisOutput="")}; var cur=""
            for(ch in text){cur+=ch;_state.update{it.copy(jarvisOutput=cur)};delay(28L+(0..18).random())}
        }
    }
    ...
    private fun startIdle() {
        ...
        idleJob=viewModelScope.launch{var i=0;while(isActive){typeText(IDLE[i%IDLE.size]);i++;delay(4_500)}}
    }
    private fun stopIdle() { idleJob?.cancel();idleJob=null }

    private fun addLog(msg:String,level:String) {
        val lv=when(level){"ok"->EventLevel.OK;"warn"->EventLevel.WARN;"err"->EventLevel.ERR;else->EventLevel.INFO}
        val e=LogEvent(logFmt.format(Date()),msg,lv)
        _state.update{it.copy(eventLog=(it.eventLog+e).takeLast(50))}
    }
