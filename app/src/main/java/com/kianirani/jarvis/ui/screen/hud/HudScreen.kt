...
// ── Data models ──────────────────────────────────────────────────────────────
data class NodeInfo(val id:String, val name:String, val ip:String, val role:String,
                    val online:Boolean, val cpu:Int, val ram:Int, val ping:Int)
...
@Composable fun LandscapeLayout(s: HudUiState, vm: HudViewModel) {
    Column(Modifier.fillMaxSize()) {
        ...
        Row(Modifier.fillMaxWidth().weight(1f).padding(12.dp), horizontalArrangement=Arrangement.spacedBy(12.dp)) {
            ...
            Column(Modifier.width(210.dp).fillMaxHeight(), verticalArrangement=Arrangement.spacedBy(8.dp)) {
                RadarPanel(s, Modifier.fillMaxWidth().height(170.dp))
...
@Composable fun TopBar(s: HudUiState, vm: HudViewModel, modifier: Modifier) {
    Row(modifier.height(52.dp)
        ...
        .border(BorderStroke(1.dp, Brush.horizontalGradient(listOf(Color.Transparent,JarvisColors.Border,Color.Transparent))))
        ...
        Row(horizontalArrangement=Arrangement.spacedBy(8.dp)) {
            listOf("BRAIN" to s.brainOnline, "NODES" to (s.nodesOnline==s.nodes.size&&s.nodes.isNotEmpty()), "GROQ" to s.groqOnline).forEach { (l,ok) ->
                Column(Modifier.border(1.dp,(if(ok)JarvisColors.NeonGreen else JarvisColors.DangerRed).copy(.3f),RoundedCornerShape(3.dp))
                    .background((if(ok)JarvisColors.NeonGreen else JarvisColors.DangerRed).copy(.05f),RoundedCornerShape(3.dp))
                    ...
                    Text(if(ok)"OK" else "ERR", style=MaterialTheme.typography.labelSmall, color=if(ok)JarvisColors.NeonGreen else JarvisColors.DangerRed)
                }
            }
        }
        Row(verticalAlignment=Alignment.CenterVertically, horizontalArrangement=Arrangement.spacedBy(10.dp)) {
            Box(Modifier.clip(RoundedCornerShape(3.dp))
                ...
                Text(if(s.isListening)"◉ MIC" else "○ MIC", style=MaterialTheme.typography.labelMedium,
    ...
    HudCard(modifier) {
        Canvas(Modifier.fillMaxSize().padding(28.dp)) {
            ...
            rotate(ring,Offset(cx,cy)){
                ...
                val dx=cx+maxR*.88f; drawCircle(JarvisColors.CyanPrimary,4.dp.toPx(),Offset(dx,cy))
                drawCircle(JarvisColors.CyanGlow,8.dp.toPx(),Offset(dx,cy))
            }
            rotate(ring2,Offset(cx,cy)){
                ...
                repeat(3){ i -> val a=(i*120f)*(PI/180f).toFloat()
                    val px=cx+maxR*.65f*cos(a); val py=cy+maxR*.65f*sin(a)
...
@Composable fun HudCard(modifier:Modifier=Modifier, content:@Composable BoxScope.()->Unit) {
    Box(modifier.clip(RoundedCornerShape(4.dp))
        .background(Brush.linearGradient(listOf(Color(0xF5061525),Color(0xFB020B18))))
...
@Composable fun BracketDeco(color:Color=JarvisColors.CyanSecondary,size:Dp=9.dp,sw:Dp=1.5.dp) {
    Canvas(Modifier.fillMaxSize()){
        val s=size.toPx(); val p=4.dp.toPx(); val w=sw.toPx(); val c=color
        val W=this.size.width; val H=this.size.height
        listOf(Pair(Offset(p,p),Pair(Offset(p+s,p),Offset(p,p+s))),
               Pair(Offset(W-p,p),Pair(Offset(W-p-s,p),Offset(W-p,p+s))),
               Pair(Offset(p,H-p),Pair(Offset(p+s,H-p),Offset(p,H-p-s))),
               Pair(Offset(W-p,H-p),Pair(Offset(W-p-s,H-p),Offset(W-p,H-p-s)))).forEach{(o,e)->
...
@Composable fun NodeCard(n:NodeInfo,modifier:Modifier) {
    ...
    HudCard(modifier){Column(Modifier.padding(10.dp)){
        Row(Modifier.fillMaxWidth(),Arrangement.SpaceBetween,Alignment.CenterVertically){
            ...
            Text(if(n.online)"ONLINE" else "OFFLINE",style=MaterialTheme.typography.labelSmall,color=col)
        }
        Spacer(Modifier.height(3.dp))
        Text("${n.ip} · ${n.role}",style=MaterialTheme.typography.bodySmall,color=JarvisColors.TextDim)
        Spacer(Modifier.height(7.dp))
        Row(horizontalArrangement=Arrangement.spacedBy(14.dp)){
            listOf("CPU" to "${n.cpu}%","RAM" to "${n.ram}%","PING" to "${n.ping}ms").forEach{(l,v)->
...
@Composable fun MetricPanel(s:HudUiState,modifier:Modifier) {
    HudCard(modifier){Column(Modifier.padding(10.dp)){
        ...
        MetricBar("I/O",s.brainDiskIo,JarvisColors.WarningAmber)
    }}
}
    ...
    Column(Modifier.padding(bottom=7.dp)){
        ...
        Box(Modifier.fillMaxWidth().height(2.dp).background(JarvisColors.GridLine,RoundedCornerShape(1.dp))){
            Box(Modifier.fillMaxWidth((v/100f).coerceIn(0f,1f)).fillMaxHeight()
                .background(Brush.horizontalGradient(listOf(color.copy(.5f),color)),RoundedCornerShape(1.dp)))
        }
    }
}
...
@Composable fun RadarPanel(s:HudUiState,modifier:Modifier){
    HudCard(modifier){Column(Modifier.padding(10.dp)){
        Text("SYSTEM RADAR",style=MaterialTheme.typography.labelSmall,color=JarvisColors.TextDim,modifier=Modifier.padding(bottom=4.dp))
        RadarCanvas(listOf(s.brainCpu/100f,s.brainRam/100f,s.brainNet/100f,0.45f,0.62f),
            listOf("CPU","RAM","NET","TEMP","I/O"),Modifier.fillMaxSize())
    }}
}

@Composable fun RadarCanvas(vals:List<Float>,labels:List<String>,modifier:Modifier){
    Canvas(modifier){
        val cx=size.width/2;val cy=size.height/2;val r=minOf(cx,cy)*.72f;val n=vals.size
        val step=(2*PI/n).toFloat()
        repeat(4){i->
            val gr=r*((i+1)/4f);val path=Path()
            repeat(n){j->val a=step*j-(PI/2).toFloat()
                val px=cx+gr*cos(a);val py=cy+gr*sin(a)
                if(j==0)path.moveTo(px,py) else path.lineTo(px,py)}
            path.close();drawPath(path,JarvisColors.GridLine,style=Stroke(.8.dp.toPx()))
        }
        repeat(n){j->val a=step*j-(PI/2).toFloat()
            drawLine(JarvisColors.GridLine,Offset(cx,cy),Offset(cx+r*cos(a),cy+r*sin(a)),.7.dp.toPx())}
        val dp=Path()
        vals.forEachIndexed{j,v->val a=step*j-(PI/2).toFloat()
            val px=cx+r*v*cos(a);val py=cy+r*v*sin(a)
...
@Composable fun LogPanel(s:HudUiState,modifier:Modifier){
    HudCard(modifier){Column(Modifier.padding(10.dp)){
        ...
        s.eventLog.takeLast(10).forEach{e->
...
@Composable fun InputBar(value:String,onChange:(String)->Unit,onSend:()->Unit,modifier:Modifier){
    Row(modifier.border(1.dp,JarvisColors.Border,RoundedCornerShape(4.dp))
        ...
        BasicTextField(value,onChange,Modifier.weight(1f),
            ...
            decorationBox={inner->Box{
                if(value.isEmpty())Text("Send command to JARVIS Brain...",style=MaterialTheme.typography.bodyMedium,color=JarvisColors.TextDim)
                inner()}})
...
@Composable fun WaveformBar(amps:List<Float>,active:Boolean,modifier:Modifier){
    Row(modifier.background(Brush.horizontalGradient(listOf(JarvisColors.CyanFaint,Color.Transparent,JarvisColors.CyanFaint)))
        .border(BorderStroke(1.dp,Brush.horizontalGradient(listOf(Color.Transparent,JarvisColors.Border,Color.Transparent))))
        .padding(horizontal=14.dp),
        verticalAlignment=Alignment.CenterVertically,horizontalArrangement=Arrangement.spacedBy(10.dp)){
        Text("VOICE I/O",style=MaterialTheme.typography.labelSmall,color=JarvisColors.TextDim)
        Canvas(Modifier.weight(1f).fillMaxHeight().padding(vertical=6.dp)){
            ...
            amps.forEachIndexed{i,a->
                val bh=mh*a.coerceIn(.02f,1f); val x=i*(bw+3.dp.toPx()); val y=(mh-bh)/2f
                drawRect(Brush.verticalGradient(
                    if(active)listOf(JarvisColors.CyanPrimary,JarvisColors.BlueDeep) else listOf(JarvisColors.CyanSecondary.copy(.3f),JarvisColors.BlueDeep.copy(.2f)),
                    startY=y,endY=y+bh),Offset(x,y),Size(bw,bh))
            }
        }
        Column(horizontalAlignment=Alignment.End){
            Text("FFT",style=MaterialTheme.typography.labelSmall,color=JarvisColors.TextDim)
            Text("48kHz",style=MaterialTheme.typography.labelSmall,color=JarvisColors.NeonGreen)
        }
    }
}
...
@Composable fun ScanLine(modifier:Modifier){
    ...
    Canvas(modifier.alpha(.3f)){
        drawRect(Brush.horizontalGradient(listOf(Color.Transparent,JarvisColors.CyanPrimary,Color.Transparent)),
            Offset(0f,size.height*y/100f),Size(size.width,3.dp.toPx()))
    }
}

@Composable fun CornerBrackets(modifier:Modifier){
    Canvas(modifier){
        val s=44.dp.toPx();val w=2.dp.toPx();val c=JarvisColors.CyanSecondary.copy(.45f)
        val W=size.width;val H=size.height
        listOf(Pair(Offset(0f,0f),Pair(Offset(s,0f),Offset(0f,s))),
               Pair(Offset(W,0f),Pair(Offset(W-s,0f),Offset(W,s))),
               Pair(Offset(0f,H),Pair(Offset(s,H),Offset(0f,H-s))),
               Pair(Offset(W,H),Pair(Offset(W-s,H),Offset(W,H-s)))).forEach{(o,e)->
            drawLine(c,o,e.first,w);drawLine(c,o,e.second,w)}
    }
}
