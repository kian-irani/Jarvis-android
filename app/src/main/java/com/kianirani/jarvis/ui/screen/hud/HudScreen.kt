...
@Composable fun TopBar(s: HudUiState, vm: HudViewModel, modifier: Modifier) {
    Row(modifier.height(52.dp).background(Brush.horizontalGradient(listOf(JarvisColors.CyanFaint, Color.Transparent, JarvisColors.CyanFaint))).padding(horizontal = 14.dp),
        ...
        Row(verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.spacedBy(10.dp)) {
            Box(Modifier.size(32.dp).clip(RoundedCornerShape(4.dp)).background(Brush.linearGradient(listOf(JarvisColors.BlueAccent, JarvisColors.BlueDeep))).border(1.dp, JarvisColors.CyanSecondary, RoundedCornerShape(4.dp)), Alignment.Center) {
        ...
        Row(horizontalArrangement = Arrangement.spacedBy(6.dp)) {
            listOf("BRAIN" to s.brainOnline, "NODES" to (s.nodesOnline > 0), "GROQ" to s.groqOnline).forEach { (label, ok) ->
                ...
                Column(Modifier.border(1.dp, color.copy(alpha = 0.3f), RoundedCornerShape(3.dp)).background(color.copy(alpha = 0.05f), RoundedCornerShape(3.dp)).padding(horizontal = 8.dp, vertical = 3.dp), horizontalAlignment = Alignment.CenterHorizontally) {
    ...
    HudCard(modifier) {
        Canvas(Modifier.fillMaxSize().padding(28.dp)) {
            ...
            rotate(ring2, Offset(cx, cy)) {
                ...
                repeat(3) { i -> val a = (i * 120f) * (PI / 180f).toFloat(); drawCircle(JarvisColors.BlueDeep, 3.dp.toPx(), Offset(cx + maxR * 0.65f * cos(a), cy + maxR * 0.65f * sin(a))) }
            }
        ...
        Column(Modifier.align(Alignment.BottomCenter).padding(bottom = 12.dp), horizontalAlignment = Alignment.CenterHorizontally) {
            Text(if (s.isListening) "LISTENING" else "STANDBY", style = MaterialTheme.typography.labelMedium, color = if (s.isListening) JarvisColors.NeonGreen.copy(alpha = 0.5f + blink * 0.5f) else JarvisColors.CyanSecondary.copy(alpha = 0.7f))
        }
    }
}

@Composable fun HudCard(modifier: Modifier = Modifier, content: @Composable BoxScope.() -> Unit) {
    Box(modifier.clip(RoundedCornerShape(4.dp)).background(Brush.linearGradient(listOf(Color(0xF5061525.toInt()), Color(0xFB020B18.toInt())))).border(1.dp, JarvisColors.Border, RoundedCornerShape(4.dp))) { BracketDeco(); content() }
}
...
@Composable fun BracketDeco() {
    Canvas(Modifier.fillMaxSize()) {
        ...
        listOf(Triple(Offset(p,p), Offset(p+s,p), Offset(p,p+s)), Triple(Offset(W-p,p), Offset(W-p-s,p), Offset(W-p,p+s)),
               Triple(Offset(p,H-p), Offset(p+s,H-p), Offset(p,H-p-s)), Triple(Offset(W-p,H-p), Offset(W-p-s,H-p), Offset(W-p,H-p-s))).forEach { (o, h, v) -> drawLine(c, o, h, w); drawLine(c, o, v, w) }
    }
}
...
@Composable fun TypewriterPanel(text: String, modifier: Modifier) {
    ...
    HudCard(modifier) { Column(Modifier.padding(12.dp)) {
        ...
        Row { Text(">> ", style = MaterialTheme.typography.bodyMedium, color = JarvisColors.CyanPrimary); Text(text, style = MaterialTheme.typography.bodyMedium, color = JarvisColors.TextPrimary, maxLines = 3, overflow = TextOverflow.Ellipsis); Text("|", style = MaterialTheme.typography.bodyMedium, color = JarvisColors.CyanPrimary.copy(alpha = cur)) }
    } }
}
    ...
    HudCard(modifier) { Column(Modifier.padding(10.dp)) {
        Row(Modifier.fillMaxWidth(), Arrangement.SpaceBetween, Alignment.CenterVertically) {
            Row(verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.spacedBy(6.dp)) {
                Box(Modifier.size(12.dp), Alignment.Center) { Box(Modifier.size(10.dp).scale(sc).clip(CircleShape).background(col.copy(alpha = 0.25f))); Box(Modifier.size(6.dp).clip(CircleShape).background(col)) }
        ...
        Row(horizontalArrangement = Arrangement.spacedBy(12.dp)) {
            listOf("CPU" to "${n.cpu}%", "RAM" to "${n.ram}%").forEach { (label, value) -> Column { Text(label, style = MaterialTheme.typography.labelSmall, color = JarvisColors.TextDim); Text(value, style = MaterialTheme.typography.titleLarge, color = JarvisColors.CyanPrimary) } }
        }
    } }
}
...
@Composable fun MetricPanel(s: HudUiState, modifier: Modifier) {
    HudCard(modifier) { Column(Modifier.padding(10.dp)) {
        ...
        MetricBar("CPU", s.brainCpu, JarvisColors.CyanPrimary); MetricBar("RAM", s.brainRam, JarvisColors.BlueDeep); MetricBar("NET", s.brainNet, JarvisColors.NeonGreen)
    } }
}
...
@Composable fun MetricBar(label: String, value: Float, color: Color) {
    ...
    Column(Modifier.padding(bottom = 7.dp)) {
        Row(Modifier.fillMaxWidth(), Arrangement.SpaceBetween) { Text(label, style = MaterialTheme.typography.labelSmall, color = JarvisColors.TextDim); Text("${v.toInt()}%", style = MaterialTheme.typography.labelSmall, color = color) }
        Spacer(Modifier.height(3.dp))
        Box(Modifier.fillMaxWidth().height(2.dp).background(JarvisColors.GridLine, RoundedCornerShape(1.dp))) { Box(Modifier.fillMaxWidth((v / 100f).coerceIn(0f, 1f)).fillMaxHeight().background(Brush.horizontalGradient(listOf(color.copy(alpha = 0.5f), color)), RoundedCornerShape(1.dp))) }
    }
}
    ...
    HudCard(modifier) { Column(Modifier.padding(10.dp)) {
        ...
        Canvas(Modifier.fillMaxSize()) {
            ...
            repeat(4) { i -> val gr = r * ((i + 1) / 4f); val path = Path()
                repeat(n) { j -> val a = step * j - (PI / 2).toFloat(); if (j == 0) path.moveTo(cx + gr * cos(a), cy + gr * sin(a)) else path.lineTo(cx + gr * cos(a), cy + gr * sin(a)) }
                path.close(); drawPath(path, JarvisColors.GridLine, style = Stroke(0.8.dp.toPx())) }
            val dp = Path(); vals.forEachIndexed { j, v -> val a = step * j - (PI / 2).toFloat(); if (j == 0) dp.moveTo(cx + r * v * cos(a), cy + r * v * sin(a)) else dp.lineTo(cx + r * v * cos(a), cy + r * v * sin(a)) }
...
@Composable fun LogPanel(s: HudUiState, modifier: Modifier) {
    HudCard(modifier) { Column(Modifier.padding(10.dp)) {
        ...
        s.eventLog.takeLast(8).forEach { e -> Row(Modifier.padding(bottom = 3.dp), horizontalArrangement = Arrangement.spacedBy(5.dp)) {
            Text(e.time, style = MaterialTheme.typography.bodySmall, color = JarvisColors.TextDim)
            Text(e.message, style = MaterialTheme.typography.bodySmall, maxLines = 1, overflow = TextOverflow.Ellipsis, color = when (e.level) { EventLevel.OK -> JarvisColors.NeonGreen; EventLevel.WARN -> JarvisColors.WarningAmber; EventLevel.ERR -> JarvisColors.DangerRed; else -> JarvisColors.CyanSecondary })
        } }
    } }
}
...
@Composable fun WaveformBar(amps: List<Float>, active: Boolean, modifier: Modifier) {
    Row(modifier.background(Brush.horizontalGradient(listOf(JarvisColors.CyanFaint, Color.Transparent, JarvisColors.CyanFaint))).padding(horizontal = 14.dp),
        ...
        Canvas(Modifier.weight(1f).fillMaxHeight().padding(vertical = 6.dp)) {
            ...
            amps.forEachIndexed { i, a -> val bh = mh * a.coerceIn(0.02f, 1f); val x = i * (bw + 3.dp.toPx()); val y = (mh - bh) / 2f
...
@Composable fun HexGrid(modifier: Modifier) {
    Canvas(modifier) {
        ...
        repeat((size.height / hh).toInt() + 2) { row -> repeat((size.width / hw).toInt() + 2) { col ->
            val cx = col * hw + (if (row % 2 == 1) hw / 2 else 0f); val cy = row * hh * 0.75f; val path = Path()
            repeat(6) { i -> val a = (60 * i - 30) * (PI / 180f).toFloat(); if (i == 0) path.moveTo(cx + hw/2*cos(a), cy + hw/2*sin(a)) else path.lineTo(cx + hw/2*cos(a), cy + hw/2*sin(a)) }
...
@Composable fun CornerBrackets(modifier: Modifier) {
    Canvas(modifier) {
        val s = 44.dp.toPx(); val w = 2.dp.toPx(); val c = JarvisColors.CyanSecondary.copy(alpha = 0.45f); val W = size.width; val H = size.height
        listOf(Triple(Offset(0f,0f), Offset(s,0f), Offset(0f,s)), Triple(Offset(W,0f), Offset(W-s,0f), Offset(W,s)),
               Triple(Offset(0f,H), Offset(s,H), Offset(0f,H-s)), Triple(Offset(W,H), Offset(W-s,H), Offset(W,H-s))).forEach { (o, h, v) -> drawLine(c, o, h, w); drawLine(c, o, v, w) }
    }
}
